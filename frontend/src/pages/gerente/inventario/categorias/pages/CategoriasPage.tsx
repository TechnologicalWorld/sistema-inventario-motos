...
export default function CategoriasPage() {
  ...
  const [selected, setSelected] = useState<Categoria | null>(
    null
  );
  const [form, setForm] =
    useState<CategoriaFormState>(emptyForm);
  ...
  const handleDelete = async (cat: Categoria) => {
    ...
    try {
      await categoriasService.deleteCategoria(
        cat.idCategoria
      );
      await loadCategorias();
    } catch (error: any) {
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          <h1 className="text-2xl font-semibold text-gray-800">
            Lista de tipos de productos registrados en el
            sistema
          </h1>
          ...
          <div className="bg-[#f5ede8] w-full max-w-xl rounded-md shadow-lg border border-gray-400">
            <div className="bg-[#e5ddda] px-6 py-3 rounded-t-md text-left">
              <h2 className="font-semibold text-lg text-gray-800">
                ...
                {String(selected.idCategoria).padStart(
                  3,
                  "0"
                )}
              </h2>
