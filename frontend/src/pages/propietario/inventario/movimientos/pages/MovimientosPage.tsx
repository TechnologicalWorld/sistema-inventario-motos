...
export default function MovimientosPage() {
  ...
  const movimientosFiltrados = movimientos.filter((m) => {
    ...
    const respName = `${m.empleado?.persona?.nombres} ${
  ...
  const getTipoClase = (tipo: string) => {
    if (tipo === "entrada") return "text-green-600 font-semibold";
    if (tipo === "salida") return "text-red-600 font-semibold";
        ...
        <table className="w-full text-sm">
          <thead className="bg-gray-300">
            <tr className="text-left">
              <th className="px-4 py-2 font-semibold">Fecha Movimiento</th>
            ...
            {!loading &&
              movimientosFiltrados.map((m, idx) => (
                ...
                >
                  ...
                  <td className="px-4 py-2">{m.observacion}</td>
                  <td className="px-4 py-2">
                    <button
          ...
          <div className="bg-[#f5ede8] w-full max-w-xl rounded-md shadow-lg border border-gray-400">
            ...
            <div className="px-6 py-4 space-y-2 text-sm">
              <div className="grid grid-cols-2 gap-y-2">
                ...
                <span>{formatFecha(selected.fechaMovimiento)}</span>

                <span className="font-semibold">Tipo:</span>
                <span>{getTipoTexto(selected.tipo)}</span>

                <span className="font-semibold">Producto:</span>
                <span>{selected.producto?.nombre}</span>
